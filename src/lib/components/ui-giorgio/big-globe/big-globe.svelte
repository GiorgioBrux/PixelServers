...
$: {
  ...
  const cosAngle = Math.sin(phi0) * Math.sin(phi) +
                   Math.cos(phi0) * Math.cos(phi) * Math.cos(lambda0 - lambda);
  ...
  return cosAngle > 0;
	 ...
	 */
	 function dragged(event: { dx: any; dy: any }) {
        ...
        rotation = [
            (currentRotation[0] + dx * scale + 360) % 360,
            Math.max(-90, Math.min(90, currentRotation[1] - dy * scale)),
            currentRotation[2]
        ];
...
<svg class="block h-full w-full" viewBox="320 0 300 500" preserveAspectRatio="xMidYMid meet">
